...
export default async function TeacherTimetablePage() {
  ...
  let maxPeriod = 4;
  let classesPerDay: Record<string, number> = {};
  ...
  try {
    ...
    if (teacher) {
      school = teacher.classes[0]?.schoolGrade?.school;
      allSchedules = teacher.classes.flatMap((c: any, idx: number) =>
      ...
      maxPeriod = Math.max(...allSchedules.map((s: any) => s.periodNumber), school?.sessionsPerDay || 4);
      classesPerDay = DAYS.reduce((acc: Record<string, number>, day: string) => {
        acc[day] = allSchedules.filter((s: any) => s.dayOfWeek === day).length;
        return acc;
      }, {} as Record<string, number>);
      totalWeeklyClasses = allSchedules.length;
    }
  } catch (err: any) {
    console.error("Timetable page error:", err?.message || err);
  }
  ...
  return (
    <>
      ...
      <div className="p-6 lg:p-8 space-y-6">
        ...
        {allSchedules.filter((s: any) => s.dayOfWeek === todayDay).length > 0 && (
          <div className="card border-brand-200 bg-brand-50/30">
            ...
            <div className="flex gap-2 flex-wrap">
              {allSchedules.filter((s: any) => s.dayOfWeek === todayDay).sort((a: any, b: any) => a.periodNumber - b.periodNumber).map((s: any) => (
                <div key={s.id} className={`p-3 rounded-xl border ${s.colorClass} min-w-[150px]`}>
                  ...
                  <div className="text-[9px] opacity-60 mt-0.5">
                    ...
                    <span className={`ml-1 text-[7px] px-1 rounded ${s.sessionSlot === "SESSION_B" ? "bg-blue-500 text-white" : s.sessionSlot === "SESSION_C" ? "bg-purple-500 text-white" : "bg-amber-500 text-white"}`}>
                      {s.sessionSlot === "SESSION_B" ? "PM" : s.sessionSlot === "SESSION_C" ? "Eve" : "AM"}
                    </span>
                  </div>
                </div>
              ))}
        ...
        {allSchedules.length === 0 ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full border-collapse min-w-[600px]">
              ...
              <tbody>
                {Array.from({ length: maxPeriod }, (_, i) => i + 1).map((period: number) => (
                  <tr key={period}>
                    <td className="p-2 border border-gray-200 text-center">
                      <div className="text-[10px] font-bold text-gray-600">P{period}</div>
                    </td>
                    {DAYS.slice(0, 5).map((day: string) => {
                      const entry = allSchedules.find((s: any) => s.dayOfWeek === day && s.periodNumber === period);
                      return (
