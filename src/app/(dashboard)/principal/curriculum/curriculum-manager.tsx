...
const ALL_GRADES = [
  "K1","K2","K3","G1","G2","G3","G4","G5","G6","G7","G8","G9","G10","G11","G12"
];
...
export default function CurriculumManager({ grades }: { grades: any[] }) {
  ...
  const handleRemoveGrade = async (gradeId: string) => {
    ...
    setLoading(gradeId);
    await removeGradeLevel(gradeId);
    router.refresh();
    setLoading("");
  };
  ...
  const handleRemoveSubject = async (gsId: string) => {
    ...
    setLoading(gsId);
    await removeSubjectFromGrade(gsId);
    router.refresh();
    setLoading("");
  };

  const selectCommonSubject = (subj: { name: string; code: string }) => {
    setSubjectForm((p) => ({ ...p, name: subj.name, code: subj.code }));
  };
    ...
    <div className="space-y-6">
      ...
      <div className="card">
        ...
        <div className="flex gap-3">
          <select className="input-field flex-1" value={newGrade} onChange={(e) => setNewGrade(e.target.value)}>
            ...
            {ALL_GRADES.filter((g) => !existingGrades.has(g)).map((g) => (
      ...
      {grades.length === 0 ? (
        <div className="card text-center py-12">
          ...
          <p className="text-gray-500">No grade levels added yet. Add your first grade level above.</p>
        </div>
      ) : (
            ...
            <div key={grade.id} className="card">
              <div className="flex items-center justify-between cursor-pointer" onClick={() => setExpandedGrade(expandedGrade === grade.id ? null : grade.id)}>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h4 className="text-sm font-semibold text-gray-800">{GRADE_LABELS[grade.gradeLevel] || grade.gradeLevel}</h4>
                    <p className="text-xs text-gray-500">{grade.subjects.length} subjects</p>
                  </div>
                </div>
                    ...
                    <div className="p-4 bg-blue-50 rounded-lg space-y-3">
                      <div className="flex flex-wrap gap-1 mb-2">
                        {COMMON_SUBJECTS.map((s) => (
                          <button key={s.code} type="button" onClick={() => selectCommonSubject(s)}
