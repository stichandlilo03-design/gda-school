...
import { BookOpen, Users, Clock, Award, GraduationCap, AlertCircle, CheckCircle, XCircle } from "lucide-react";
...
export default async function StudentDashboard() {
  ...
  const student = await db.student.findUnique({
    ...
    include: {
      school: { select: { name: true, motto: true, primaryColor: true } },
      enrollments: {
        where: { status: "ACTIVE" },
        include: {
          class: {
            include: { teacher: { include: { user: { select: { name: true } } } }, schoolGrade: true },
          },
        },
      },
      certificates: true,
      ...
      <div className="p-6 lg:p-8 space-y-8">
        ...
        {isPending && (
          <div className="flex items-center gap-4 p-5 bg-amber-50 border border-amber-200 rounded-xl">
            ...
            <div>
              ...
              <p className="text-sm text-amber-600">Your application to {student.school.name} is awaiting approval from the school principal. You'll have full access once approved.</p>
            </div>
          </div>
        )}
        ...
        {isRejected && (
          <div className="flex items-center gap-4 p-5 bg-red-50 border border-red-200 rounded-xl">
            ...
            <div>
              <h3 className="text-base font-bold text-red-800">Application Not Approved</h3>
              <p className="text-sm text-red-600">Unfortunately, your application was not approved. Please contact the school for more information.</p>
            </div>
          </div>
        )}
                    ...
                    <Link key={enrollment.id} href={`/student/classroom?classId=${enrollment.classId}`}
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-gray-500">
                          Teacher: {enrollment.class.teacher.user.name} • {enrollment.class.schoolGrade.gradeLevel}
                        </p>
                      </div>
        ...
        {/* School Info */}
        <div className="card">
          <h3 className="section-title mb-3">School</h3>
          <p className="text-sm font-semibold text-gray-800">{student.school.name}</p>
          {student.school.motto && <p className="text-xs text-gray-500 italic">"{student.school.motto}"</p>}
        </div>
      </div>
    </>
  );
